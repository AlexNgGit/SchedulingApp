...
int Project::addDependecies(int rowCount, XLWorksheet currSheet) {
    for (auto element: input) {
        int subElementIndex = 2;
        while(subElementIndex <= rowCount) {
            vector<string> dependenciesStr;
            auto taskName = currSheet.cell("D" + subElementIndex).value().typeAsString();
            splitComma(taskName, dependenciesStr);
            for (auto subElement: dependenciesStr) {
                auto element =  findTask(subElement);
                if (element != nullptr && element != NULL) {
                    element->addParentNode(findTask(subElement));
                }
            }
        }
    }
...
int Project::loadExcelFiles(string path) {
    ...
    addDependecies(subIndex, currSheet);
}
